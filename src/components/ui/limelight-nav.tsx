...
export const LimelightNav = ({
  ...
  return (
    <nav
      className={`relative inline-flex items-center h-16 rounded-xl bg-card/80 text-foreground border border-zinc-700/60 px-2 backdrop-blur-xl ${className}`}
    >
      ...
      <div
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full bg-yellow-400 shadow-[0_50px_15px_rgba(250,204,21,0.42)] ${
          isReady ? "transition-[left] duration-400 ease-in-out" : ""
        } ${limelightClassName}`}
        style={{ left: "-999px" }}
      >
        <div className="absolute left-[-30%] top-[5px] h-14 w-[160%] [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-yellow-300/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
